...
AGENT_CMD=${AGENT_CMD:-'/usr/local/bin/vmagent-prod -remoteWrite.tmpDataPath /tmp/vmagent-remotewrite-data -remoteWrite.url=http://METRIC_SERVER:8428/api/v1/write -enableTCP6 -promscrape.config /tmp/scrape_config.yaml -graphiteListenAddr localhost:2003 -remoteWrite.relabelConfig /tmp/write_relabel.yaml'}
...
METRIC_SERVER=$1
echo "Metric server specified is: ${METRIC_SERVER}"
...
WantedBy=multi-user.target
EOF
  sudo sed -i "s|METRIC_SERVER|$METRIC_SERVER|g" /tmp/metrics_agent.service
